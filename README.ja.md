...
- タイトルバーを隠した付箋は左端に帯が付きます。折りたたむとタイトルバー表示の付箋と同じ1行になるため、その見分けを兼ねます。設定の **タイトルバー非表示の目印** で表示の有無と太さ（1〜12px）を変更できます。
...
![編集モードの付箋。タイトルバーのボタンと編集ツールバー](docs/toolbar.png)
...
![本文の右クリックメニューと、その上に出るミニツールバー](docs/context-menu.png)
...
![トレイアイコンの右クリックメニュー](docs/tray-menu.png)
...
| 設定 | 新規付箋の既定値、テーマ、言語、付箋の見た目、自動起動、タスクバー・トレイ動作、保存先 |
| 外部ファイルを付箋として開く | `.md` / `.txt` を読み取り専用で表示し、変更に追従 |

![設定画面](docs/settings.png)

付箋の見た目は **設定 → 表示** でまとめて変更できます。角の丸み（0〜16px。既定は「丸めない」）、枠の色（既定は「付箋の色」。なし／グレーも選べます）、アイコンの色（カラー／モノクロ）、タイトルバー非表示の目印。変更は開いている付箋にその場で反映されます。

枠の色を「なし」にすると、白い背景の上では淡い色の付箋の輪郭がほとんど見えなくなります。

0.2.0 以前から更新した場合、初回起動で見た目が変わります（丸い角とグレーの枠 → 角のまま・付箋の色の枠）。以前の見た目に戻すには、**設定 → 表示** で「角の丸み: 6」「枠の色: グレー」を選んでください。

...
付箋のメニューまたは付箋一覧からリマインダーを設定できます。1回のみ・毎日・毎週（複数曜日）・毎月（1〜31日）を選べます。指定日が存在しない月は月末に通知します。日時欄は繰り返しの開始日時として使います。

Windowsの標準通知を表示し、クリックすると付箋一覧を開きます。同時刻の通知は1つにまとめます。必要ならスヌーズできる通知ウィンドウも併用できます。スヌーズしても繰り返し時刻は変わりません。

通知にはScreenPinNotesの起動（トレイ常駐）が必要です。終了・休止中に過ぎた予定は再開時に1回通知して、次の予定へ進みます。Windowsの通知設定によってバナーや音が抑制される場合があります。
...
- 折りたたみ表示
- タイトルバー非表示モード（操作アイコンは右上にホバー表示、折りたたみ時は本文1行目を表示、アイコン未設定なら移動用のつまみを表示）
- 設定画面（新しい付箋の既定値、表示、動作、データ）
...
編集中は付箋の左下に「編集中」を表示します。フォーカスが外れてツールバーが隠れても、編集中であることが分かります。
...
タスクトレイの **設定** から設定画面を開きます。新しい付箋の既定値（色・フォント・フォントサイズ・アイコン・タイトルバーの有無）、表示（ダークモード、言語）、動作（スタートアップ登録、タスクバー表示、トレイアイコンのクリック動作、折りたたみ中の本文プレビュー、折りたたみ・展開のアニメーション、折りたたみ・展開ボタン、ダブルクリックでの表示切替）、データ（保存フォルダ、エクスポート、インポート）をまとめています。変更はその場で反映して保存します。タスクバー表示をONにすると各付箋のウィンドウがタスクバーにも表示され、そこから選択するとその付箋が最前面に来ます。トレイアイコンの左クリックの動作は、全付箋の表示・非表示切り替え（既定）と新規付箋の追加から選べます。

新しい付箋の既定値は、タスクトレイから作った付箋に使います。既存の付箋の「＋」から増やしたときは、そちらの見た目を引き継ぎます。

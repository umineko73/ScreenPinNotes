...
予定や買い物リスト、Markdownの表、写真を並べた日本語の表示例。付箋の色はテーマに関係なくライト10色・ダーク10色から選べ、背景色とアクセント色を自由に決めるカスタム配色も使えます。
...
| 画像 | 画像の貼り付けや画像ファイルのドロップで保存・表示。拡大縮小、画像に合わせた付箋サイズの調整、絵・ファイルとしてのコピー |
| draw.io | 図面を埋め込んだ PNG や `.drawio` を右クリックして draw.io で編集。保存すると付箋の絵も自動で入れ替わります |
| ファイル | 画像以外のファイルをドロップすると、エクスプローラーと同じアイコンの札として並び、クリックで開けます。Shift 付きならコピーせず元の場所を指します |
...
| リマインダー | 一度だけ・毎日・毎週（第n週の指定も可）・毎月（最終日も可）の通知。付箋の点滅、Windows のサウンド、スヌーズにも対応 |
| 付箋一覧 | 検索、表示・非表示の切り替え、重なり順やリマインダーの管理 |
| 外部ファイル | `.md` / `.txt` / `.log` を読み取り専用の付箋として開き、ファイルの変更を反映（開いたまま追記され続けるログにも追従。最短更新間隔を設定可能）。`.log` は既定で tail モードになり、末尾N行だけをプレーンテキストで表示して常に最新行へ自動スクロール |
...
3. `Ctrl+Alt+N`、またはトレイアイコンの右クリックメニューから付箋を作成し、そのまま入力します。`Ctrl+Alt+Shift+N` でクリップボードの文字・画像から付箋を作れます。
...
| タイトルバーをダブルクリック、または山形（上 / 下）のボタン | 折りたたみ・展開 |
| ピン | 最前面への固定を切り替え |
...
設定から保存先を変更し、zip形式のバックアップを書き出し・取り込みできます。同名の付箋フォルダーがある場合は、添付ファイルを含めて上書きする・フォルダー名を自動で変えて両方を残す・取り込まない、の3つから選べます。`SCREENPINNOTES_DATA` 環境変数で別のデータフォルダーを指定することもできます。

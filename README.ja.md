...
3. トレイアイコンを右クリックして付箋を作成。本文をダブルクリックすると編集できます。
...
Windows 10以降・x64。インストール不要。角丸表示はWindows 11に対応します。
...
通常の移動では、展開・折りたたみの位置を連動させます。位置を分けた付箋は **折りたたみ時の位置にそろえる** で連動に戻せます。各表示の幅は別々に記憶します。
...
設定は即時保存されます。新規付箋の既定値はトレイからの作成に適用され、既存付箋の `＋` は元の見た目を引き継ぎます。

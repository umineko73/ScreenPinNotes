...
Windows may not report changes while the writing program keeps the file open, as loggers usually do. The note therefore also checks the file's size and modification time every second. It stops checking after a minute without changes and starts again when Windows reports a change, or when you point at or click the note. While it is checking, a small dot at the right end of the title fades with each check, and the tooltip says *Checking for changes*. The interval and the idle time can be changed with `ExternalFile.PollIntervalMs` and `ExternalFile.PollStopAfterMs` in `settings.json`.

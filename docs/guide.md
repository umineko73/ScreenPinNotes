...
| Body in view mode | Copy, go to or copy a link (on a link), copy Excel tables, fit the window to images |
...
- Selecting text while editing shows a formatting toolbar above it (cut, copy, paste, bold, italic, strikethrough, highlight, inline code, bulleted and numbered lists, checklist, link, and clear formatting, which also removes the line's heading/list/quote marker). Right-click **Markdown formatting** offers the same plus headings and quotes. **Edit link** changes a link's label and URL.

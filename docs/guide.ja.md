...
ログのように書き込み中のアプリがファイルを開いたままだと、Windows から変更の通知が届かないことがあります。そのためファイルのサイズと更新日時も1秒ごとに確認しています。1分間変化がなければ確認を止め、Windows から変更の通知が届いたときや、付箋にマウスを乗せた・クリックしたときに再開します。間隔と止めるまでの時間は `settings.json` の `ExternalFile.PollIntervalMs` と `ExternalFile.PollStopAfterMs` で変更できます。

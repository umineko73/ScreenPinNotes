...
ログのように書き込み中のアプリがファイルを開いたままだと、Windows から変更の通知が届かないことがあります。そのためファイルのサイズと更新日時も確認しています（変更を見つけてから30秒間は1秒ごと、それ以外は5秒ごと）。
...
ファイルの中身が実際に変わったときは、付箋の枠が水色に1回光って知らせます。届いた行に `ERROR` `FATAL` が含まれていたときは、枠が赤く光り、付箋の背景色がその色から赤へ半分寄った色との間で3回点滅します（文字は覆いません）。その付箋を操作している最中は光りません（見ている本人には更新が届いているため）。中身が変わらない保存（更新日時だけ触るような書き込み）では読み直しも点滅もしません。

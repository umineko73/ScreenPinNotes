...
- 編集中に文字列を選ぶと、その上に書式ツールバーが出ます（切り取り・コピー・貼り付け・太字・斜体・取り消し線・ハイライト・インラインコード・箇条書き・番号付きリスト・チェックリスト・リンク・修飾の削除。修飾の削除は行頭の見出し・箇条書き・引用などの印も外します）。右クリックの **Markdown書式** では、これらに加えて見出し・引用も挿入できます。**リンクを編集** で表示名とURLを変更できます。

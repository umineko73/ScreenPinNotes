...
通常の移動では、展開・折りたたみの位置を連動させます。位置を分けた付箋は **折りたたみ時の位置にそろえる** で連動に戻せます。位置を分けた付箋を開いているあいだは、折りたたむと戻る場所にグレーの影が表示されます。影をクリックするとその場所に折りたたみ、ドラッグすると折りたたみ時の位置だけを動かせます。右クリックで位置をそろえられます。影にマウスを乗せると、対応する付箋が縁取られます。影は設定の「折りたたみ」で非表示にできます。各表示の幅は別々に記憶します。隣接する付箋へのスナップでは、物理1ピクセルの隙間を残します。
...
- 編集中に文字列を選ぶと、その上に書式ツールバーが出ます（切り取り・コピー・太字・斜体・取り消し線・ハイライト・インラインコード・箇条書き・番号付きリスト・チェックリスト・リンク・修飾の削除。修飾の削除は行頭の見出し・箇条書き・引用などの印も外します）。右クリックの **Markdown書式** では、これらに加えて見出し・引用も挿入できます。**リンクを編集** で表示名とURLを変更できます。
...
**付箋を30秒間点滅** は初期状態で有効です。非表示の付箋も表示し、付箋全体をオレンジ色でゆっくり点滅させます（本文は透けて読めます）。点滅する付箋は他のアプリの窓より前に出ます（**設定 → 動作 → リマインダー** でオフにできます。他の付箋は前に出ません）。クリック・キー入力・非表示で停止します。Windows通知、スヌーズ付き通知ウィンドウ、点滅、サウンドは組み合わせて選択でき、点滅だけ・サウンドだけにもできます。

**サウンドを30秒間繰り返す** は新しいリマインダーでは初期状態で有効です。鳴らす音は **設定 → 動作 → リマインダーの音** で、Windows のサウンド（`C:\Windows\Media` の .wav）から選びます。**試聴** で確かめられます。

点滅と音の長さは **設定 → 動作 → 点滅・サウンドの長さ（秒）** で変えられます（初期値30秒、1〜600秒）。付箋をクリックするかキーを押すと、点滅と音はすぐ止まります。通知ウィンドウを出したときは、それを閉じると音が止まります。

通知にはトレイ常駐が必要です。終了・休止中に過ぎた予定は再開後に1回通知します。Windows通知のバナーと通知音はWindowsの通知設定に従います。

...
| 設定 | 新規付箋の既定値、テーマ、言語、付箋の見た目、自動起動、タスクバー・トレイ動作、外部ファイルの更新間隔・tail 表示、保存先 |
...
外部ファイル付箋は `🔗` で識別でき、タイトルバーには最後に読み直した日時が表示されるので、内容が最新かどうか一目で分かります。ファイルを開く・フォルダを開く・tail モードの切り替え・編集できる付箋に変換する操作があります。付箋の削除や画像表示サイズの変更で元ファイルは変更されません。

tail モード以外では、自動更新のたびにできるだけスクロール位置とキャレット位置を保ち、先頭に戻ってしまわないようにしています。

tail モードでは、行の中の `INFO` `DEBUG` `TRACE` を緑、`WARN` を橙、`ERROR` `FATAL` を赤で表示します（大文字小文字は問いません）。色を付けるのは各行で最初に見つかったレベル名だけです。数値は青で表示し、日時や件数を目で追いやすくしています。`2026-09-15` や `09:12:03.221`、`16/16` のように区切りで繋がった数値はひとまとまりとして扱います。

ファイルの中身が実際に変わったときは、付箋の枠が水色に1回光って知らせます。届いた行に `ERROR` `FATAL` が含まれていたときは赤く光ります。その付箋を操作している最中は光りません（見ている本人には更新が届いているため）。中身が変わらない保存（更新日時だけ触るような書き込み）では読み直しも点滅もしません。

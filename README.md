...
本文は Markdown で書けます。編集時は Markdown ソースをそのまま表示し、閲覧モードでは見出し、太字、斜体、インラインコード、コードブロック、箇条書き、番号付きリスト、チェックリスト、引用、水平線、表、画像、Markdownリンクを整形して表示します。チェックリストは閲覧モードのままクリックして切り替えられます。
...
編集モードで画像を貼り付けると、付箋フォルダ内の `assets` に PNG として保存し、本文には `![image](assets/{ファイル名}.png)` を挿入します。閲覧モードでは画像として表示されます。

Markdown画像記法は `![説明](assets/image.png)` の形式に対応しています。相対パスはその付箋フォルダから解決されます。閲覧モードで画像を右クリックすると、元画像サイズ、付箋の表示可能幅、画像の最大表示高さに収まるサイズを 100% として、10% から 100% まで 10% 刻みでサイズを選べます。サイズ指定は `![説明](assets/image.png){width=240}` のように本文へ保存されます。

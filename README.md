...
普段は閲覧モードで、本文中の URL やフォルダパスをシングルクリックで開けます。本文をクリックすると編集モードに切り替わり、操作ボタンが下部のステータスバーに現れます。Escape かフォーカスを外すと閲覧モードに戻ります。
...
```
artifacts/
  ScreenStickyNotes-0.0.1-win-x64.exe            68.3 MB  自己完結型
  ScreenStickyNotes-0.0.1-win-x64-runtime.exe     0.2 MB  ランタイム必須
```

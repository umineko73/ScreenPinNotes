...
編集中に文字列を選択すると、その上に小さなツールバーが出ます。切り取り、コピー、貼り付け、太字、斜体、取り消し線、ハイライト、インラインコード、箇条書き、番号付きリスト、チェックリスト、リンクと、Tₓ で修飾の削除（文字の修飾・リンクと、行頭の見出し・箇条書き・引用などの印を外す）ができます。右クリックの **Markdown書式** では、これらに加えて見出し1〜3と引用も選べます。行の書式は選択がかかる行全体に適用します。太字などは選択がないと記号の間にカーソルを置き、対応する同じ書式の再選択で解除できます。`Ctrl+Z` で元に戻せます。
...
**付箋を10秒間点滅** は初期状態で有効です。非表示の付箋も表示し、付箋全体をオレンジ色でゆっくり点滅させます（本文は透けて読めます）。クリック・キー入力・非表示で停止します。Windows通知、スヌーズ付き通知ウィンドウ、点滅は組み合わせて選択でき、点滅だけにもできます。
